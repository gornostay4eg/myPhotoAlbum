...
router.get('/addalbum', async (req, res) => {
...
router.post('/addalbum', async (req, res) => { 
    try {
        ...
        return res.redirect('/albums'); // - после создания нового альбома делаем редирект на страничку этого альбома
    } catch (error) {
        return res.send('Error');
    }
});
...
router.get(':id', async (req, res) => { 
    try {
        const photos = await Photo.findAll({
            where: { user_id: req.session.user.id, album_id: req.params.id } });
            return res.render('myalbum', {photos});
        } catch (error) {
            return res.send('Error');
        }
    });
